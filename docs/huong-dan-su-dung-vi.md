...
- Một agent CLI: Codex, Claude Code, Cursor Agent hoặc generic JSON-protocol command.
...
## 4. Task mơ hồ hoặc chưa có thiết kế
...
Generic agent phải nhận một JSON request từ stdin và trả đúng một JSON response từ stdout. Log người đọc phải đi qua stderr. Capability phải khai báo đúng implementer, testgen hoặc reviewer.
...
## 17. Frontend và backend ở hai repository
...
## 17. Playbook case thực tế mở rộng
...
## 13. Luồng Harness Engine dành cho người không muốn viết YAML
...
Nếu task mơ hồ, CLI trả về tối đa ba câu hỏi. Trả lời bằng interactive terminal hoặc `--set`:
...
## 14. Chạy nhiều task và phục hồi worker
...
## 15. Đánh giá model và phát hiện regression
...
## 16. Khi không ưng UI hoặc cần human approval
