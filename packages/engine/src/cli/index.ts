...
import { loadBacklog, allSlicesPass } from "../core/backlog.js";
...
program
  ...
  .action(() => {
    ...
    if (fs.existsSync(configPath)) {
      logger.warn("sliceforge.config.json already exists. Skipping config initialization.");
    } else {
      const defaultConfig = {
    ...
    if (fs.existsSync(backlogPath)) {
      logger.warn("whole-app-backlog.json already exists. Skipping backlog initialization.");
    } else {
      ...
      fs.writeFileSync(backlogPath, JSON.stringify(defaultBacklog, null, 2), "utf8");
...
program
  .command("once")
  .description("Run a single Ralph iteration (pick -> implement -> verify)")
  .action(async () => {
    const cwd = process.cwd();
    const config = loadConfig(cwd);
...
program
  .command("testgen")
  .description("Run the TestGen Loop to generate test cases from spec documents")
...
program
  ...
  .action(() => {
    ...
    try {
      ...
      console.log(`Backlog Completion: ${completed}/${total} slices (${pct}%)`);
      ...
      if (state.costAccumulated.estimatedCostUSD !== undefined) {
        console.log(`API Cost Accumulated: $${state.costAccumulated.estimatedCostUSD.toFixed(4)}`);
      }

      console.log("\nSlices list:");
      for (const slice of backlog.slices) {
        const marker = slice.passes ? "\x1b[32m[x]\x1b[0m" : "\x1b[33m[ ]\x1b[0m";
        console.log(`  ${marker} ${slice.id} (Priority: ${slice.priority}) - ${slice.description}`);
      }
    } catch (err: any) {
      logger.error(`Failed to retrieve status: ${err.message}`);
    }
  });
...
program
  ...
  .description("Approve and commit a slice currently pending human review")
